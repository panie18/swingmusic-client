...
// No static baseURL; set per request from current server config
...
api.interceptors.request.use(async (config) => {
  // baseURL per request
  config.baseURL = ensureTrailingSlash(getBaseUrl());
  ...
  async (error) => {
    ...
    if (error?.response?.status === 401) {
      ...
      try {
        ...
        const url = ensureTrailingSlash(getBaseUrl()) + "auth/refresh";
